...
for (const variant of ['breaking', 'compatible'] as const) test(`two bundled native apps: ${variant} checklist saves, shared rollout and same-row rollback`, { skip: !nativeAvailable, timeout: 60_000 }, async () => {
  ...
  async function boot(index: number, release: string) {
    ...
    const child = spawn(process.execPath, ['--import', 'tsx', 'server.ts'], { cwd, env: { ...process.env,
      GECCO_DATABASE_URL: databases[index].url, GECCO_ADMIN_TOKEN: token, GECCO_RELEASE: release, GECCO_PROPOSED_CHECKOUT: undefined, GECCO_STATE_FILE: app.stateFile,
  ...
  try {
    ...
    // These assertions concern the immutable bundled fixtures. The independent
    // proposal validator pairs checked-out release code with its own migrations.
    const leftBoot = await boot(0, 'v1');
    const rightBoot = await boot(1, `v2-${variant}`);
    assert.equal(leftBoot.release, 'v1');
    assert.equal(leftBoot.releaseSelection, 'override');
    assert.equal(rightBoot.releaseSelection, 'override');
    assert.equal(leftBoot.releaseEntryPoint, 'engine/specimen/v1.ts');
    assert.equal(rightBoot.releaseEntryPoint, `engine/specimen/v2-${variant}.ts`);
